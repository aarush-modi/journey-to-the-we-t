...
public class WizardController : MonoBehaviour, IDamageable
{
    ...
    private void Update()
    {
        ...
        Debug.Log($"[Wizard] dist={dist:F1} attackRange={attackRange} detectRange={detectRange} canAttack={canAttack} isAttacking={isAttacking} isHurt={isHurt}");
    ...
    private IEnumerator Attack()
    {
        ...
        if (!isDead && player != null)
        {
            ...
            string compNames = "";
            foreach (var c in comps) compNames += c.GetType().Name + ",";
            Debug.Log($"[Wizard] playerObj={player.gameObject.name}, target={target != null}, components={compNames}");
